...
//clear function to remove all inputs
...
function changeSelect() {
    ...
    current = sel.selectedIndex;//gets the index
    document.getElementById("Pname").value = arrPromName[current];//changes the form to the selected promotions data
    ...
    switch (arrDur[current]) {
        case (1):
            document.getElementById("duration1").checked;
            break;
        case (2):
            document.getElementById("duration2").checked;
            break;
        case (3):
            document.getElementById("duration3").checked;
            break;
        default:
            alert("did not work");
            break;
    ...
    document.getElementById("Pprice").value = arrPrice[current];

    document.getElementById("Cname").value = arrName[current];
    document.getElementById("Cnum").value = arrCell[current];
    document.getElementById("Cemail").value = arrEmail[current];
}//end of function
...
function createEventListeners() {
    ...
    if (btnEdit.addEventListener) {
        btnEdit.addEventListener("click", passwordCheck, false);
    } else if (btnEdit.attachEvent) {
        btnEdit.attachEvent("onclick", passwordCheck);
    }
    //delete button
    var btnDelete = document.getElementById("delete");
    if (btnDelete.addEventListener) {
        btnDelete.addEventListener("click", passwordCheck1, false);
    } else if (btnDelete.attachEvent) {
        btnDelete.attachEvent("onclick", passwordCheck1);
    }
    //summary button
    var btnSum = document.getElementById("btnSummary");
    if (btnSum.addEventListener) {
        btnSum.addEventListener("click", openSum, false);
    } else if (btnSum.attachEvent) {
        btnSum.attachEvent("onclick", openSum);
    }
}
